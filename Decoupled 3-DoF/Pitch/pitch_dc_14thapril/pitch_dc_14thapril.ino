...
// --------------------------
float latestPitch = 0.0;  // Changed from roll to pitch
...
// Data Logging
// --------------------------
// Logs in the order: pitch, angular velocity, m1, m2, m3, m4
void captureDataSample(float pitch, float angVel, int m1, int m2, int m3, int m4) {
  String line = String(pitch, 2) + "," + String(angVel, 2) + "," +
  ...
  if(now - lastControlTime >= 10) {
    ...
    while(bno08x.getSensorEvent(&sensorValue)) {
      if(sensorValue.sensorId == SH2_ROTATION_VECTOR) {
        ...
        // Instead of roll, we now use pitch:
        latestPitch = pitch;
      }
      else if(sensorValue.sensorId == SH2_GYROSCOPE_CALIBRATED) {
        // Use the fused gyroscope reading on the y-axis (pitch rate) and convert rad/s to deg/s
        latestAngularVelocity = sensorValue.un.gyroscope.y * (180.0 / PI);
      }
    }
    ...
    if(experimentMode == "random") {
      ...
      if(totalElapsed >= randomMaxDuration) {
        ...
        if(randomCycleActive) {
          ...
          captureDataSample(latestPitch, latestAngularVelocity, latestM1, latestM2, latestM3, latestM4);
        }
      }
    }
