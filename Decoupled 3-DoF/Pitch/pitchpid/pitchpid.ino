...
// ------------------------------------
static double desiredPitch = 0.0; // Desired pitch angle, set via WebSocket command
bool armed = false;
...
float latestGyroRate = 0.0; // Angular velocity (deg/s) about Y-axis (pitch)
...
void webSocketEvent(uint8_t num, WStype_t type, uint8_t * payload, size_t length) {
  if (type == WStype_TEXT) {
    ...
    // Desired Pitch command: "R:<angle>"
    else if (cmd.startsWith("R:")) {
      String angleStr = cmd.substring(2);
      desiredPitch = angleStr.toFloat();
      Serial.println("[!] Desired Pitch set to " + String(desiredPitch, 2));
      webSocket.sendTXT(num, "Desired Pitch set to " + String(desiredPitch, 2));
    }
...
void loop() {
  ...
  if (now - lastControlTime >= 10) {
    ...
    while (bno08x.getSensorEvent(&sensorValue)) {
      if (sensorValue.sensorId == SH2_ROTATION_VECTOR) {
        // Update pitch from the rotation vector
        ...
        latestGyroRate = sensorValue.un.gyroscope.y * (180.0 / PI);
      }
    }
    ...
    if (armed) {
      ...
      // ---------------------------
      double angleError = desiredPitch - latestPitch;
  ...
  if (now - lastTelemetryTime >= 100) {
    ...
    // Changed references from 'roll' to 'pitch'
    String telemetry = "A_ang:" + String(latestPitch, 2) +
                       "A_vel:" + String(velocityMeasured, 2);
    /*
    String telemetry = "T_ang:" + String(desiredPitch, 2) +
                       ",A_ang:" + String(latestPitch, 2) +
                       ",E_ang:" + String(desiredPitch - latestPitch, 2) +
