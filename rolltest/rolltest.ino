...
void setup() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.println("\n❌ WiFi connection failed. Check SSID/password or hotspot.");
  }
  ...
  unsigned long sensorTimeout = millis();
  while (!sensorReady && millis() - sensorTimeout < 5000) {
    sensorReady = bno08x.begin_I2C();
    if (!sensorReady) {
      Serial.print(".");
      delay(200);
    }
  ...
  if (!sensorReady) {
    Serial.println("\n❌ BNO08x not detected. Continuing without sensor.");
  } else {
    Serial.println("\n✅ BNO08x detected!");
    ...
    if (!bno08x.enableReport(SH2_ROTATION_VECTOR, 20)) {
      ...
      Serial.println("✅ Rotation vector report enabled at 20 ms");
    }

    if (!bno08x.enableReport(SH2_GYROSCOPE, 20)) {
      Serial.println("❌ Could not enable calibrated gyroscope report");
    } else {
      Serial.println("✅ Calibrated gyroscope report enabled at 20 ms");
    }
  }
